...
[claude.ts askClaude]
   ...
   ├─ embed(baseQuery, rewritten)  → Voyage API  ← ембединг питання (512d)
   ├─ retrieveMulti (retriever.ts)               ← cosine по index.json, поріг 0.45,
   │      → топ-14 чанків, ≤1 на статтю, ФОП/ЮО відсіяні
   ├─ є чанки? → Claude (Sonnet) з document-блоками + citations:enabled
   │             нема? → Claude + web_search (allowed_domains, max_uses 1)
   ▼
[claude.ts] збираємо джерела = ТІЛЬКИ процитовані документи
[bot.ts] прибираємо вже показані в цій темі (conversation.ts) → надсилаємо відповідь
```

Логи в терміналі/Render: `[Rewrite]`, `[Index]`, `[RAG] знайдено N чанків`, `[Claude] in/out tokens | sources`.
...
data/raw/*.md   (+ data/curated/*.md — ручні документи)
   ...
   ▼
data/index.json  (~88 МБ, 12k чанків, 512-вимірні вектори)
```
...
| `409 Conflict` | бот политься з двох місць | не запускай локально, поки живий Render (один токен) |
